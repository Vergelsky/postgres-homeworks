...
SELECT MIN(unit_price) AS min, MAX(unit_price) AS max FROM products WHERE discontinued <> 1;
...
SELECT MIN(unit_price) AS min, MAX(unit_price) AS max FROM products WHERE discontinued <> 1 AND units_in_stock > 20;